...
export default function ExplorePage() {
  ...
  useEffect(() => {
    const fetchExplore = async () => {
      // Mocking explore data since backend is down
      const mockImages = Array.from({ length: 18 }).map((_, i) =>
        `https://picsum.photos/seed/explore${i}/600/600`
      );
          ...
          {filteredImages.map((url, index) => {
            ...
            return (
              <div
                ...
                className={`relative overflow-hidden cursor-pointer group ${isLarge ? "row-span-2 col-span-1" : ""
                  }`}
              >
                <img
                  src={url}
                  alt={`Explore ${index}`}
                  className={`w-full ${isLarge ? "h-full" : "aspect-square"
